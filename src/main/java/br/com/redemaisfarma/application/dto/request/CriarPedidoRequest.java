...
public class CriarPedidoRequest implements Serializable {
    ...
    @Size(max = 200, message = "{pedido.itens.size}") // evita payloads gigantes
    @Valid
    @JsonProperty("itens")
    ...
    public CriarPedidoRequest(UUID pedidoId, Long clienteId, List<ItemPedidoRequestDTO> itens, String enderecoEntrega,
            BigDecimal valorTotal, FormaPagamentoRequestDTO formaPagamento, String tenantId, UUID traceId,
            LocalDateTime dataCriacao, String observacao, LocalDateTime dataAtualizacao) {
    ...
    // ------------------ Consistência de dados (Bean Validation) ------------------

    @AssertTrue(message = "{pedido.totais.coerencia}")
    @JsonProperty(access = JsonProperty.Access.READ_ONLY)
    public boolean isTotaisCoerentes() {
        if (itens == null || itens.isEmpty() || valorTotal == null)
            return false;
        BigDecimal soma = itens.stream().map(i -> i.getPrecoUnitario().multiply(BigDecimal.valueOf(i.getQuantidade())))
    ...
    public boolean isFormaPagamentoIgualTotal() {
        if (formaPagamento == null || valorTotal == null || formaPagamento.getValor() == null)
            return false;
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(pedidoId, that.pedidoId) && Objects.equals(clienteId, that.clienteId)
                && Objects.equals(itens, that.itens) && Objects.equals(enderecoEntrega, that.enderecoEntrega)
                && Objects.equals(valorTotal, that.valorTotal) && Objects.equals(formaPagamento, that.formaPagamento)
                && Objects.equals(tenantId, that.tenantId) && Objects.equals(traceId, that.traceId)
                && Objects.equals(dataCriacao, that.dataCriacao) && Objects.equals(observacao, that.observacao)
                && Objects.equals(dataAtualizacao, that.dataAtualizacao);
    }

    @Override
    public int hashCode() {
        return Objects.hash(pedidoId, clienteId, itens, enderecoEntrega, valorTotal, formaPagamento, tenantId, traceId,
                dataCriacao, observacao, dataAtualizacao);
    }

    @Override
    public String toString() {
        return "CriarPedidoRequest{" + "pedidoId=" + pedidoId + ", clienteId=" + clienteId + ", itens=" + itens
                + ", enderecoEntrega='" + enderecoEntrega + '\'' + ", valorTotal=" + valorTotal + ", formaPagamento="
                + formaPagamento + ", tenantId='" + tenantId + '\'' + ", traceId=" + traceId + ", dataCriacao="
                + dataCriacao + ", observacao='" + observacao + '\'' + ", dataAtualizacao=" + dataAtualizacao + '}';
    }
}
