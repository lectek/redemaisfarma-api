...
public record ProductCardVM(
    ...
    public static List<ProductCardVM> fromListInOrder(List<ProdutoEntity> ps, List<Long> orderIds) {
        if (ps == null || ps.isEmpty() || orderIds == null || orderIds.isEmpty()) return fromList(ps);
        Map<Long, ProductCardVM> map = new HashMap<>();
        ps.forEach(p -> map.put(p.getId(), of(p)));
        List<ProductCardVM> out = new ArrayList<>();
        for (Long id : orderIds) {
            ProductCardVM vm = map.get(id);
    ...
    // Agora direto no campo real (sem reflexão)
