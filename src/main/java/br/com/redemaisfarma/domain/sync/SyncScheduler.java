...
public class SyncScheduler {
    ...
    public SyncScheduler(LegacyProdutoPort legacy,
...
public void run() {
    LocalDateTime since = checkpoint.findSinceOrEpoch(SOURCE);
    ...
    while (true) {
        ...
        try {
            lote = legacy.fetchChangedSince(since, page, pageSize);
        } catch (Exception e) {
            log.error("Falha no fetchChangedSince(page={}): {}", page, e.getMessage(), e);
            break; // ou continue; dependendo da estratégia
        }
        ...
        for (LegacyProdutoDTO r : lote) {
            if (r.nome() == null || r.precoVenda() == null
                    || r.precoVenda().compareTo(BigDecimal.ZERO) <= 0) {
                continue;
            }

            ProdutoEntity p = repo.findByCodigoBarras(r.ean())
            ...
            p.setLegacyId(r.legacyId()); // ✅ correção mantida
        ...
        if (lote.size() < pageSize) break; // 👉 última página
    }
