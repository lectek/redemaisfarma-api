...
- **Cadastro simplificado**: o endpoint `POST /api/auth/register` agora só exige e-mail e senha; nome e CPF são opcionais e preenchidos automaticamente (nome vindo da parte local do e-mail, CPF gerado quando ausente). Essa automação facilita criação rápida de contas e mantém validações (e-mail único, senha forte) intactas.
